...
exports.routes = [{
        ...
        run(req, res) {
            ...
            if (Match) {
                ...
                let data = prepareObject(Match, req.baseUrl);
                data.matches = [];
                if (type !== "emotes" && Match.setParts && Match.setParts[0]) {
                    ...
                    Match.setParts.forEach(part => {
                        let matches = false;
                        let asset = Assets[types(part.split(":")[1])].filter(a => a.id == part.split(":")[0])[0];
                        ...
                        if (matches) data.matches.push({
                            id: asset.id,
                            type: part.split(":")[1]
                        });
                    });
                    if (global.exceptions) {
                        if (global.exceptions.additions && global.exceptions.additions[0]) {
                            let possibleExceptions = global.exceptions.additions.filter(e => e.id == Match.id && e.updates[0])[0];
                            if (possibleExceptions) {
                                possibleExceptions.updates.forEach(m => {
                                    data.matches.push({
                                        id: m.split(":")[0],
                                        type: m.split(":")[1]
                                    });
                        ...
                        if (global.exceptions.replacements && global.exceptions.replacements[0]) {
                            let possibleExceptions = global.exceptions.replacements.filter(e => e.id == Match.id && e.updates[0])[0];
                            if (possibleExceptions) {
                                possibleExceptions.updates.forEach(m => {
                                    let origin = data.matches.filter(d => d.id == m.origin)[0];
                                    if (!origin) return null;
                                    data.matches.filter(d => d.id == m.origin)[0].id = m.new;
                                });
                            };
                        };
                    };
    ...
    },
    {
        ...
        run(req, res) {
            ...
            if (!types(req.headers.type)) return res.status(400).json({ statusCode: 400, msg: "Invalid type" });
            let type = types(req.headers.type);
            let item = Assets[type].filter(a => a.id == req.headers.item.toLowerCase())[0];
