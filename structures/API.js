  ...
  return new Promise((resolve, reject) => {
    fetch(ENDPOINTS.BENBOT.AES)
      .catch(err => {
        ...
        if (aes) {
          console.log(
            '[Warning] Could not fetch AES encryption keys from ' +
            ENDPOINTS.BENBOT.AES +
            ". Using the key you've provided in your config."
          )
          return resolve({
            mainKey: aes
          })
...
export async function getHotfix () {
  ...
  if (
    !cloudstorage.filter(
      file => file.filename.toLowerCase() === 'defaultgame.ini'
    )[0]
  ) {
    return []
  }
  const hotfixUri =
    ENDPOINTS.FORTNITE.CLOUDSTORAGE +
    '/' +
    cloudstorage.filter(
      file => file.filename.toLowerCase() === 'defaultgame.ini'
    )[0].uniqueFilename
  const hotfix = await fetch(hotfixUri, {
    headers: { 'User-Agent': build.fortnite.UserAgent, Authorization: token }
  }).then(res => res.text())
  if (!hotfix) return []
  return hotfix
    .split('\n')
